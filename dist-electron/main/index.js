import { app, ipcMain, BrowserWindow, shell, nativeImage, Tray } from "electron";
...
function controlWindow(window, tray2) {
  const toggle = () => {
    if (window.isVisible())
      window.hide();
    else
...
async function App() {
  ...
  const { toggle } = controlWindow(window, tray2);
  window.setAlwaysOnTop(true);
  tray2.on("click", toggle);
}
